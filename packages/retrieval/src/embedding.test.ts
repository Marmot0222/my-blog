...
const vector = () => Array.from({ length: 2048 }, () => 0);

test("Embedding 配置与聊天配置解耦并固定 2048 维", () => {
  ...
  assert.equal(config.dimensions, 2048);
...
test("验证 Embedding 数量、维度与有限数值", () => {
  ...
  assert.throws(() => validateEmbeddings([[0, 1]], 1), /期望 2048 维，实际 2 维/);
...
test("仅向官方 OpenAI 接口显式请求数据库固定的向量维度", () => {
  assert.deepEqual(createEmbeddingProviderOptions({ provider: "openai", dimensions: 2048 }), {
    openai: { dimensions: 2048 },
  });
  assert.equal(
    createEmbeddingProviderOptions({ provider: "openai-compatible", dimensions: 2048 }),
    undefined,
  );
});
